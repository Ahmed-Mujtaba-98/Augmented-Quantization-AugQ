...
class AdaRoundQuantizer(nn.Module):
    ...
    def forward(self, x):
        if self.round_mode == 'nearest':
            ...
            data_quant = torch.where((self.mini < x) & (x < self.step[0]),
                                     self.quantize(x, self.zero_point[0], self.aug_delta[0]),
                                     x
                                     )
            data_quant = torch.where((self.step[0] < x) & (x < self.step[1]),
                                     self.quantize(x, self.zero_point[1], self.aug_delta[1]),
                                     data_quant
                                     )
            data_quant = torch.where((self.step[1] < x) & (x < self.step[2]),
                                     self.quantize(x, self.zero_point[2], self.aug_delta[2]),
                                     data_quant
                                     )
            data_quant = torch.where((self.step[2] < x) & (x < self.maxi),
                                     self.quantize(x, self.zero_point[3], self.aug_delta[3]),
                                     data_quant
                                     )
        else:
    ...
    def quantize(self, x, zero_point, delta):
